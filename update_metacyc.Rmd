...
exampleReactions <- compoundsReactions %>% 
  dplyr::filter(compoundID %in% extract2(linkedExampleCompound, 'Compound'))
...
```{r message=FALSE, warning=FALSE}
genesIDs <- read_tsv(file.path(updateDir, '4_genes_and_IDs.tsv'))
glimpse(genesIDs)
genesIDs %<>% 
  mutate(webEnsembl = Ensembl) %>% mutate(webGeneCards = GeneCards) %>% 
  ...
  select(-`Accession-1`, -ends_with('End-Position'))
glimpse(genesIDs)
```

And join to the table
```{r}
exampleGenesIDs <- left_join(exampleGenes, genesIDs, by = 'geneID')
glimpse(exampleGenesIDs)
```
...
```{r}
pathwaysReactions <- read_tsv(file.path(updateDir, '5_pathways_and_reactions.tsv'))
```
